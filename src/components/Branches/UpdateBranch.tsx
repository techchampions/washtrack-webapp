...
import { useUpdateBranch } from "@/hooks/mutations/useBranch";
...
const UpdateBranch: React.FC<Props> = ({ branch }) => {
  const { closeModal } = useModal();
  const navigate = useNavigate();
  const { data: profileData, isLoading: isLoadingProfile } =
    useGetUserProfile();
  const { mutate: update, isPending } = useUpdateBranch();
  const [storeLocation, setStoreLocation] = useState(branch.store_location);
  // useEffect(() => {
  ...
  const initialValues: UpdateBranchPayload = {
    store_name: branch.store_name || "",
    phone: branch.phone || "",
    store_images: "",
    store_location: branch.store_location || "",
    state: branch.state || "",
  ...
  const validationSchema = Yup.object().shape({
    ...
    branch_id: Yup.number().required("No Parent store id"),
  });
    ...
    <div className="w-xs sm:w-md">
      <div className="mb-3 text-center">
        ...
        <p className="text-xs text-gray-500">
          Fill in the information's below to setup update your branch
        </p>
      </div>
      <Formik
      ...
      >
        {({ isValid, setFieldValue }) => (
          <Form>
            ...
            <div className="flex flex-col w-full px-3 mt-2 space-y-3">
