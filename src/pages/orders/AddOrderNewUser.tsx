...
export const AddOrderNewUser: React.FC = () => {
  ...
  const cost_of_service = orderItems.reduce(
    (x, item) => x + Number(item.price),
    0
  );
  const initialValues = {
    customerName: "",
          ...
          <div className="flex flex-col items-center w-full py-2">
            ...
            >
              {({ isValid, values, setFieldValue }) => {
                ...
                React.useEffect(() => {
                  setFieldValue("costOfService", cost_of_service);
                  // eslint-disable-next-line react-hooks/exhaustive-deps
                }, [cost_of_service, setFieldValue]);
                return (
                  <Form className="w-full space-y-2">
                    ...
                    <Button
                      ...
                      type="submit"
                      disabled={
                        !isValid || orderItems.length === 0 || isPending
                      }
                      isLoading={isPending}
                    />
                  </Form>
